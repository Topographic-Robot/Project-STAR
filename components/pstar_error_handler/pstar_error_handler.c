...
#include <string.h>
#include <stdio.h> /* For snprintf in logging */
#include "esp_log.h" /* Keep for ESP_LOGE used for MUTEX errors within the handler itself */

/* Constants ******************************************************************/

/* Fix: Use TAG consistently */
static const char* TAG = "Error Handler"; /* Renamed ERROR_HANDLER_TAG to TAG */

/* Public Functions ***********************************************************/

/* FIX: Change return type from void to esp_err_t */
...
{
  if (handler == NULL) {
    /* Use ESP_LOGE as logger cannot be assumed to be ready yet. */
    /* Cannot log error here as logger might not be initialized */
  ...
  if (handler->mutex == NULL) {
    /* Can use ESP_LOGE here as it works even before full logger init */
    ESP_LOGE(TAG, "Mutex Error - Failed to create mutex during initialization");
    return ESP_ERR_NO_MEM; /* FIX: Return error code */
  }

  return ESP_OK; /* FIX: Return success code */
}

/* FIX: Added definition for error_handler_deinit */
void error_handler_deinit(error_handler_t* handler)
{
  ...
  /* Optionally zero out the handler structure, but not strictly necessary */
  /* if (handler != NULL) { */
...
esp_err_t error_handler_record_error(error_handler_t* handler,
                                     ...
                                     const char*      file,
                                     int              line,
                                     const char*      func) /* Fix: Added func param */
{
  if (handler == NULL) {
  ...
  if (handler->mutex == NULL) {
       /* Use ESP_LOGE for mutex errors within the error handler itself. */
  ...
  /* Fix: Use timeout for mutex acquisition? Use portMAX_DELAY for now. */
  ...
  char log_buffer[CONFIG_PSTAR_KCONFIG_LOGGING_MAX_MESSAGE_LENGTH]; /* Use logger's max msg length */
  snprintf(log_buffer, sizeof(log_buffer),
           "Desc: %s | Code: %d (%s) | Loc: %s:%d (%s) | Retry: %lu/%lu",
           desc, error, esp_err_to_name(error), filename, line, funcname,
  ...
  /* bool recovery_attempted = false; // Not currently used */
  ...
  /* Fix: Modified recovery/reset logic */
  ...
  if (handler->current_retry >= handler->max_retries) {
      log_error(TAG,
                ...
                handler->max_retries, error, esp_err_to_name(error));
      ...
      if (handler->reset_fn != NULL) {
          ...
          /* recovery_attempted = true; // Not currently used */
          ...
          if (recovery_result == ESP_OK) {
              ...
              log_error(TAG,
                        ...
                        recovery_result, esp_err_to_name(recovery_result));
          }
      } else {
      ...
      /* Optionally, call reset function on every retry attempt here if desired. */
