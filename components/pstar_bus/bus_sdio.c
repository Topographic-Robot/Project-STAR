...
static esp_err_t priv_pstar_bus_sdio_write(const bus_manager_t* manager,
                                     const char*          name,
                                     const uint8_t*       data, 
                                     size_t               len,
                                     size_t               offset,
                                     size_t*              bytes_written);
static esp_err_t priv_pstar_bus_sdio_read(const bus_manager_t* manager,
                                    const char*          name,
                                    uint8_t*             data, 
                                    size_t               len,
                                    size_t               offset,
                                    size_t*              bytes_read);
static esp_err_t priv_pstar_bus_sdio_ioctl(const bus_manager_t* manager,
                                     const char*          name,
                                     int                  cmd,
                                     void*                arg);
...
void pstar_bus_sdio_init_default_ops(sdio_ops_t* ops)
{
  if (ops == NULL) {
...
esp_err_t pstar_bus_sdio_write(const bus_manager_t* manager,
                         const char*          name,
                         const uint8_t*       data, 
                         size_t               len,
                         size_t               offset,
                         size_t*              bytes_written)
{
  ...
  bus_config_t* bus_config = bus_manager_find_bus(manager, name);
  if (!bus_config) {
    log_error(bus_sdio_tag, 
              "Write Error", 
              "Bus '%s' not found", 
  ...
  if (bus_config->type != k_bus_type_sdio) {
    log_error(bus_sdio_tag, 
              "Write Error", 
              "Bus '%s' is not an SDIO bus (type: %s)", 
              name, 
              bus_type_to_string(bus_config->type));
    return ESP_ERR_INVALID_ARG;
  }
...
esp_err_t pstar_bus_sdio_read(const bus_manager_t* manager,
                        const char*          name,
                        uint8_t*             data, 
                        size_t               len,
                        size_t               offset,
...
esp_err_t pstar_bus_sdio_ioctl(const bus_manager_t* manager,
                         const char*          name,
                         int                  cmd,
                         void*                arg)
{
                                     ...
                                     void*                arg)
{
  ...
  if (card == NULL && 
      (cmd == k_sdio_ioctl_get_csd || 
       cmd == k_sdio_ioctl_get_cid || 
       cmd == k_sdio_ioctl_get_scr || 
       cmd == k_sdio_ioctl_get_ocr || 
       cmd == k_sdio_ioctl_get_rca || 
       cmd == k_sdio_ioctl_get_bus_width || 
       cmd == k_sdio_ioctl_get_bus_freq)) {
    log_error(bus_sdio_tag, 
              "IOCTL Error", 
  ...
  switch (cmd) {
    case k_sdio_ioctl_get_card_info:
      if (!arg) {
        log_error(bus_sdio_tag, 
                  "IOCTL Error", 
    ...
    case k_sdio_ioctl_get_bus_width:
      /* Get bus width */
      if (!arg) {
        log_error(bus_sdio_tag, 
                  "IOCTL Error", 
    ...
    case k_sdio_ioctl_set_bus_width:
    ...
    case k_sdio_ioctl_get_bus_freq:
      /* Get bus frequency */
      if (!arg) {
        log_error(bus_sdio_tag, 
                  "IOCTL Error", 
    ...
    case k_sdio_ioctl_set_bus_freq:
    ...
    case k_sdio_ioctl_enable_card_detect:
