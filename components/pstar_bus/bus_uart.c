 ...
 */
static esp_err_t priv_pstar_bus_uart_write(const bus_manager_t* manager,
                                     const char*          name,
                                     const uint8_t*       data, 
                                     size_t               len, 
                                     size_t*              bytes_written)
{
  ...
  if (bus_config->config.uart.callbacks.on_data_sent) {
    ...
    /* Call the callback */
    bus_config->config.uart.callbacks.on_data_sent(&event, written, bus_config->user_ctx);
  }
 ...
 * @return esp_err_t ESP_OK on success, or an error code.
 */
static esp_err_t priv_pstar_bus_uart_read(const bus_manager_t* manager,
                                    const char*          name,
                                    uint8_t*             data, 
                                    size_t               len, 
                                    size_t*              bytes_read)
{
  ...
  if (available_bytes == 0) {
    ...
    uint32_t timeout_ms = bus_config->config.uart.rx_timeout_ms;
    if (timeout_ms == 0) {
      timeout_ms = 1000; /* Default to 1000ms if not set */
    }
    ...
    if (bus_config->config.uart.callbacks.on_data_received) {
      ...
      /* Call the callback */
      bus_config->config.uart.callbacks.on_data_received(&event, data, read_bytes, bus_config->user_ctx);
    }
...
void pstar_bus_uart_init_default_ops(uart_ops_t* ops)
{
  if (ops == NULL) {
    log_error(bus_uart_tag, 
              "Init Error", 
...
esp_err_t pstar_bus_uart_write(const bus_manager_t* manager,
                         const char*          name,
                         const uint8_t*       data, 
                         size_t               len, 
                         size_t*              bytes_written)
...
esp_err_t pstar_bus_uart_read(const bus_manager_t* manager,
                        const char*          name,
                        uint8_t*             data, 
                        size_t               len, 
                        size_t*              bytes_read)
