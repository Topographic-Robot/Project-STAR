...
{
  if (ops == NULL) {
    log_error(TAG, "Init Error", "I2C operations pointer is NULL");
    return;
  }
  ...
  log_info(TAG, "Default Ops", "Initialized default I2C operations");
}
                              ...
                              size_t*                    bytes_written)
{
  ...
  if (bus_config->config.i2c.ops.write) {
    return bus_config->config.i2c.ops.write(manager, name, data, len, reg_addr, bytes_written);
  } else {
    log_error(TAG, "Write Error", "No write operation defined for I2C bus '%s'", name);
                             ...
                             size_t*                    bytes_read)
{
  ...
  if (bus_config->config.i2c.ops.read) {
    return bus_config->config.i2c.ops.read(manager, name, data, len, reg_addr, bytes_read);
  } else {
    log_error(TAG, "Read Error", "No read operation defined for I2C bus '%s'", name);
    return ESP_ERR_NOT_SUPPORTED;
  }
}
                                          ...
                                          size_t*                    bytes_written)
{
  ...
  if (cmd == NULL) {
    log_error(TAG, "Write Error", "Failed to create I2C command link");
    return ESP_ERR_NO_MEM;
  }

  /* Start condition */
  if ((result = i2c_master_start(cmd)) != ESP_OK) {
    log_error(TAG, "Write Error", "Failed to add START condition: %s", esp_err_to_name(result));
  ...
  if ((result = i2c_master_write_byte(cmd, (address << 1) | I2C_MASTER_WRITE, true)) != ESP_OK) {
    log_error(TAG, "Write Error", "Failed to write device address: %s", esp_err_to_name(result));
    i2c_cmd_link_delete(cmd);
    return result;
  }
  ...
  if ((result = i2c_master_write_byte(cmd, reg_addr, true)) != ESP_OK) {
    log_error(TAG, "Write Error", "Failed to write register address: %s", esp_err_to_name(result));
    i2c_cmd_link_delete(cmd);
    return result;
  }
  ...
  if ((result = i2c_master_write(cmd, data, len, true)) != ESP_OK) {
    log_error(TAG, "Write Error", "Failed to write data: %s", esp_err_to_name(result));
    i2c_cmd_link_delete(cmd);
    return result;
  }
  ...
  if ((result = i2c_master_stop(cmd)) != ESP_OK) {
    log_error(TAG, "Write Error", "Failed to add STOP condition: %s", esp_err_to_name(result));
    i2c_cmd_link_delete(cmd);
    return result;
  }
  ...
  if (result != ESP_OK) {
    ...
    if (bus_config->config.i2c.callbacks.on_transfer_complete) {
      ...
      pstar_bus_event_t event = {
        ...
        .bus_name = name,
        .data.i2c = {.port = port, .address = address, .is_write = true, .len = len}};

      /* Call the callback */
      bus_config->config.i2c.callbacks.on_transfer_complete(&event, bus_config->user_ctx);
    }
  }
                                         ...
                                         size_t*                    bytes_read)
{
  ...
  i2c_cmd_handle_t cmd = i2c_cmd_link_create();
  if (cmd == NULL) {
    log_error(TAG, "Read Error", "Failed to create I2C command link");
    return ESP_ERR_NO_MEM;
  }

  /* Start condition */
  if ((result = i2c_master_start(cmd)) != ESP_OK) {
    log_error(TAG, "Read Error", "Failed to add START condition: %s", esp_err_to_name(result));
    i2c_cmd_link_delete(cmd);
    return result;
  }
  ...
  /* Send device address (write mode) */
  if ((result = i2c_master_write_byte(cmd, (address << 1) | I2C_MASTER_WRITE, true)) != ESP_OK) {
    log_error(TAG, "Read Error", "Failed to write device address: %s", esp_err_to_name(result));
    i2c_cmd_link_delete(cmd);
    return result;
  }
  ...
  /* Send register address */
  if ((result = i2c_master_write_byte(cmd, reg_addr, true)) != ESP_OK) {
    log_error(TAG, "Read Error", "Failed to write register address: %s", esp_err_to_name(result));
    i2c_cmd_link_delete(cmd);
    return result;
  }
  ...
  if (len > 1) {
    if ((result = i2c_master_read(cmd, data, len - 1, I2C_MASTER_ACK)) != ESP_OK) {
      log_error(TAG, "Read Error", "Failed to read data with ACK: %s", esp_err_to_name(result));
      i2c_cmd_link_delete(cmd);
      return result;
    }
  }
  ...
  if ((result = i2c_master_read_byte(cmd, &data[len - 1], I2C_MASTER_NACK)) != ESP_OK) {
    log_error(TAG, "Read Error", "Failed to read last byte with NACK: %s", esp_err_to_name(result));
    i2c_cmd_link_delete(cmd);
    return result;
  }
  ...
  /* Stop condition */
  if ((result = i2c_master_stop(cmd)) != ESP_OK) {
    log_error(TAG, "Read Error", "Failed to add STOP condition: %s", esp_err_to_name(result));
    i2c_cmd_link_delete(cmd);
    return result;
  }
  ...
  if (result != ESP_OK) {
    ...
    if (bus_config->config.i2c.callbacks.on_transfer_complete) {
      ...
      pstar_bus_event_t event = {
        .bus_type = k_pstar_bus_type_i2c,
        .bus_name = name,
        .data.i2c = {.port = port, .address = address, .is_write = false, .len = len}};
