...
esp_err_t pstar_bus_manager_init(pstar_bus_manager_t* manager, const char* tag)
{
  ...
  log_info(manager->tag, 
           ...
           "Successfully initialized bus manager");
  return ESP_OK;
}
...
esp_err_t pstar_bus_manager_add_bus(pstar_bus_manager_t* manager, pstar_bus_config_t* config)
{
  ...
  pstar_bus_config_t* existing_bus = pstar_bus_manager_find_bus(manager, config->name);
  if (existing_bus) {
...
esp_err_t pstar_bus_manager_remove_bus(pstar_bus_manager_t* manager, const char* name)
{
  ...
  if (to_remove->initialized) {
    esp_err_t result = pstar_bus_config_deinit(to_remove);
    if (result != ESP_OK) {
  ...
  esp_err_t result = pstar_bus_config_destroy(to_remove);
  if (result != ESP_OK) {
    log_warn(manager->tag,
             "Bus destroy warning",
