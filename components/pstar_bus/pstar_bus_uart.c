...
static const char* TAG = "Bus UART"; // Renamed BUS_UART_TAG to TAG
...
{
  /* Validate input */
  // Fix: Check data and len > 0 as well
  if (!manager || !name || !data || len == 0) {
    // Fix: Use TAG
    log_error(TAG,
              "Write Error",
              "Invalid parameters: manager, name, data is NULL or len is 0");
                                           ...
                                           size_t*                    bytes_written)
{
  ...
  /* Validate input - Already done in public function */
  // if (!manager || !name || !data || len == 0) { ... }
  ...
  if (written < 0) {
    ...
    if (bus_config->config.uart.callbacks.on_error) {
        pstar_bus_event_t event = { .bus_type = k_pstar_bus_type_uart, .bus_name = name, .data.uart = { .port = port, .is_write = true, .len = 0 } };
  ...
  if ((size_t)written != len) {
       log_warn(TAG, "Write Incomplete", "uart_write_bytes wrote %d of %zu bytes to port %d", written, len, port);
       // Treat as success for now, but log warning. Adjust if full write is critical.
  }


  /* Ensure all data is transmitted before returning */
  // Fix: Use Kconfig timeout value? For now, keep 1000ms
  ...
  if (result != ESP_OK) {
    ...
    // Call error callback if available
    if (bus_config->config.uart.callbacks.on_error) {
        pstar_bus_event_t event = { .bus_type = k_pstar_bus_type_uart, .bus_name = name, .data.uart = { .port = port, .is_write = true, .len = (size_t)written } };
  ...
  if (bytes_written) {
    *bytes_written = (size_t)written; // Cast to size_t
  }
  ...
  if (bus_config->config.uart.callbacks.on_data_sent) {
    ...
    pstar_bus_event_t event = {
      ...
      .data.uart = {
        .port     = port,
        .is_write = true,
        .len      = (size_t)written // Fix: Use actual bytes written
      }
    };

    /* Call the callback */
    bus_config->config.uart.callbacks.on_data_sent(&event,
                                                   (size_t)written, // Fix: Use actual bytes written
                                                   bus_config->user_ctx);
  }
                                          ...
                                          size_t*                    bytes_read)
{
  ...
  size_t available_bytes = 0; // Initialize
  ...
  if (result != ESP_OK) {
    ...
    log_error(TAG,
     ...
     // Call error callback if available
    if (bus_config->config.uart.callbacks.on_error) {
        pstar_bus_event_t event = { .bus_type = k_pstar_bus_type_uart, .bus_name = name, .data.uart = { .port = port, .is_write = false, .len = 0 } };
        bus_config->config.uart.callbacks.on_error(&event, result, bus_config->user_ctx);
    }
    return result;
  }
  ...
  int read_bytes = 0; // Initialize
  if (to_read > 0) {
    // Read available data without blocking
    read_bytes = uart_read_bytes(port, data, to_read, 0); // Use 0 timeout
  } else {
    // No data available, wait for data with timeout
    // Fix: Use Kconfig timeout? For now, keep 1000ms default
    const uint32_t timeout_ms = 1000;
    log_debug(TAG, "Read Wait", "No data available, waiting up to %lu ms on UART port %d", timeout_ms, port);
    read_bytes = uart_read_bytes(port, data, len, pdMS_TO_TICKS(timeout_ms));

    if (read_bytes == 0) {
        // Fix: Use TAG
        log_debug(TAG, // Changed to debug as timeout isn't always an error
                 "Read Timeout",
        ...
        return ESP_ERR_TIMEOUT; // Return timeout error
    }
  }
  ...
  if (read_bytes < 0) {
    ...
    log_error(TAG,
              "Read Error",
              "uart_read_bytes failed for UART port %d",
              port);
    // Call error callback if available
    if (bus_config->config.uart.callbacks.on_error) {
        pstar_bus_event_t event = { .bus_type = k_pstar_bus_type_uart, .bus_name = name, .data.uart = { .port = port, .is_write = false, .len = 0 } };
  ...
  if (bytes_read) {
    *bytes_read = (size_t)read_bytes; // Cast to size_t
  }
  ...
  if (read_bytes > 0 && bus_config->config.uart.callbacks.on_data_received) {
    ...
    pstar_bus_event_t event = {
      ...
      .data.uart = {
        .port     = port,
        .is_write = false,
        .len      = (size_t)read_bytes // Cast to size_t
      }
    };
    ...
    bus_config->config.uart.callbacks.on_data_received(&event,
                                                       data,
                                                       (size_t)read_bytes, // Cast to size_t
                                                       bus_config->user_ctx);
  }
