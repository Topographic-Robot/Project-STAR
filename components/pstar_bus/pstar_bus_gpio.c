/* components/pstar_bus/pstar_bus_gpio.c */
...
static const char* TAG = "Bus GPIO";
...
{
  if (ops == NULL) {
    log_error(TAG, "Init Error", "GPIO operations pointer is NULL");
    return;
  }
  ...
  log_info(TAG, "Default Ops", "Initialized default GPIO operations");
}
                                   ...
                                   uint32_t                   level)
{
  /* Validate input */
  if (!manager || !name) {
    log_error(TAG, "Set Level Error", "Invalid parameters: manager or name is NULL");
  ...
  if (!bus_config) {
    log_error(TAG, "Set Level Error", "Bus '%s' not found", name);
  ...
  if (bus_config->config.gpio.ops.set_level) {
    ...
    log_error(TAG, "Set Level Error", "No set_level operation defined for GPIO bus '%s'", name);
                                   ...
                                   uint32_t*                  level)
{
  ...
  if (!manager || !name || !level) {
    log_error(TAG, "Get Level Error", "Invalid parameters: manager, name, or level is NULL");
    return ESP_ERR_INVALID_ARG;
  }
  ...
  pstar_bus_config_t* bus_config = pstar_bus_manager_find_bus(manager, name);
  if (!bus_config) {
    log_error(TAG, "Get Level Error", "Bus '%s' not found", name);
    return ESP_ERR_NOT_FOUND;
  }
  ...
  if (bus_config->config.gpio.ops.get_level) {
    ...
    log_error(TAG, "Get Level Error", "No get_level operation defined for GPIO bus '%s'", name);
    return ESP_ERR_NOT_SUPPORTED;
  }
}
                                 ...
                                 void*                      args)
{
  ...
  if (!manager || !name || !isr_handler) {
    log_error(TAG, "ISR Add Error", "Invalid parameters: manager, name, or isr_handler is NULL");
    return ESP_ERR_INVALID_ARG;
  }
  ...
  pstar_bus_config_t* bus_config = pstar_bus_manager_find_bus(manager, name);
  if (!bus_config) {
    log_error(TAG, "ISR Add Error", "Bus '%s' not found", name);
    return ESP_ERR_NOT_FOUND;
  }
  ...
  if (bus_config->config.gpio.ops.isr_add) {
    return bus_config->config.gpio.ops.isr_add(manager, name, pin_num, isr_handler, args);
  } else {
    log_error(TAG, "ISR Add Error", "No isr_add operation defined for GPIO bus '%s'", name);
    return ESP_ERR_NOT_SUPPORTED;
  }
}
...
pstar_bus_gpio_isr_remove(const pstar_bus_manager_t* manager, const char* name, gpio_num_t pin_num)
{
  /* Validate input */
  if (!manager || !name) {
    log_error(TAG, "ISR Remove Error", "Invalid parameters: manager or name is NULL");
    return ESP_ERR_INVALID_ARG;
  }
  ...
  pstar_bus_config_t* bus_config = pstar_bus_manager_find_bus(manager, name);
  if (!bus_config) {
    log_error(TAG, "ISR Remove Error", "Bus '%s' not found", name);
    return ESP_ERR_NOT_FOUND;
  }
  ...
  if (bus_config->config.gpio.ops.isr_remove) {
    ...
    log_error(TAG, "ISR Remove Error", "No isr_remove operation defined for GPIO bus '%s'", name);
    return ESP_ERR_NOT_SUPPORTED;
  }
}
                                               ...
                                               uint32_t                   level)
{
  ...
  if (!manager || !name || pin_num < 0) {
    log_error(TAG, "Set Level Error", "Invalid parameters");
    return ESP_ERR_INVALID_ARG;
  }
  ...
  if (!bus_config->initialized) {
    log_error(TAG, "Set Level Error", "Bus '%s' is not initialized", name);
  ...
  if ((bus_config->config.gpio.config.pin_bit_mask & pin_mask) == 0) {
    log_error(TAG, "Set Level Error", "Pin %d is not configured in bus '%s'", pin_num, name);
    return ESP_ERR_INVALID_ARG;
  }
  ...
  log_debug(TAG, "Set Level", "Set pin %d to level %lu on bus '%s'", pin_num, level, name);

  /* Call callback if available */
  if (bus_config->config.gpio.callbacks.on_level_change) {
    /* Create event structure */
    pstar_bus_event_t event = {.bus_type  = k_pstar_bus_type_gpio,
                               .bus_name  = name,
                               .data.gpio = {.pin_num = pin_num, .level = level}};

    /* Call the callback */
    bus_config->config.gpio.callbacks.on_level_change(&event, level, bus_config->user_ctx);
  }
                                               ...
                                               uint32_t*                  level)
{
  ...
  if (!manager || !name || pin_num < 0 || !level) {
    log_error(TAG, "Get Level Error", "Invalid parameters");
    return ESP_ERR_INVALID_ARG;
  }
  ...
  /* Check if the bus is initialized */
  if (!bus_config->initialized) {
    log_error(TAG, "Get Level Error", "Bus '%s' is not initialized", name);
    return ESP_ERR_INVALID_STATE;
  }
  ...
  uint64_t pin_mask = (1ULL << pin_num);
  if ((bus_config->config.gpio.config.pin_bit_mask & pin_mask) == 0) {
    log_error(TAG, "Get Level Error", "Pin %d is not configured in bus '%s'", pin_num, name);
    return ESP_ERR_INVALID_ARG;
  }
  ...
  if (gpio_level < 0) {
    log_error(TAG, "Get Level Error", "Failed to get level for pin %d on bus '%s'", pin_num, name);
    return ESP_FAIL;
  }
  ...
  *level = (uint32_t)gpio_level;
  ...
  log_debug(TAG, "Get Level", "Got level %lu from pin %d on bus '%s'", *level, pin_num, name);
                                             ...
                                             void*                      args)
{
  ...
  if (!manager || !name || pin_num < 0 || !isr_handler) {
    log_error(TAG, "ISR Add Error", "Invalid parameters");
    return ESP_ERR_INVALID_ARG;
  }
  ...
  /* Check if the bus is initialized */
  if (!bus_config->initialized) {
    log_error(TAG, "ISR Add Error", "Bus '%s' is not initialized", name);
    return ESP_ERR_INVALID_STATE;
  }
  ...
  uint64_t pin_mask = (1ULL << pin_num);
  if ((bus_config->config.gpio.config.pin_bit_mask & pin_mask) == 0) {
    log_error(TAG, "ISR Add Error", "Pin %d is not configured in bus '%s'", pin_num, name);
    return ESP_ERR_INVALID_ARG;
  }

  /* Check if GPIO interrupt service is not already installed
   * Attempt to install the service. If it's already installed, we'll get
   * ESP_ERR_INVALID_STATE which we can safely ignore.
   */
  ...
  log_info(TAG, "ISR Added", "Added ISR handler for pin %d on bus '%s'", pin_num, name);
                                                ...
                                                gpio_num_t                 pin_num)
{
  ...
  /* Validate input */
  if (!manager || !name || pin_num < 0) {
    log_error(TAG, "ISR Remove Error", "Invalid parameters");
    return ESP_ERR_INVALID_ARG;
  }
  ...
  /* Check if the bus is initialized */
  if (!bus_config->initialized) {
    log_error(TAG, "ISR Remove Error", "Bus '%s' is not initialized", name);
    return ESP_ERR_INVALID_STATE;
  }
  ...
  log_info(TAG, "ISR Removed", "Removed ISR handler for pin %d on bus '%s'", pin_num, name);
