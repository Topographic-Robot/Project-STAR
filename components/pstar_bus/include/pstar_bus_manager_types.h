 ...
 *
 * This structure contains all the configuration data for a specific I2C device instance.
...
typedef struct pstar_bus_config {
  ...
  pstar_bus_type_t type; /**< Type of bus (will be k_pstar_bus_type_i2c) */
  bool  initialized; /**< Whether this bus/device has been initialized via pstar_bus_config_init */
  void* handle;      /**< Driver/device handle (currently unused for I2C, kept for consistency) */
  void* user_ctx;    /**< User context pointer, passed to callbacks */

  pstar_i2c_bus_config_t i2c; /**< I2C-specific configuration (no longer a union) */

  struct pstar_bus_config* next; /**< Pointer to the next bus config in the manager's list */
} pstar_bus_config_t;

/**
 * @brief Bus manager structure that maintains a list of I2C bus configurations.
 *
 * Provides thread-safe functions to add, find, remove, and manage the lifecycle
 * of I2C bus/device configurations.
 */
