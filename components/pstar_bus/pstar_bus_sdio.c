/* components/pstar_bus/pstar_bus_sdio.c */
...
static const char* TAG = "Bus SDIO";
...
static esp_err_t
priv_pstar_bus_sdio_ioctl(const pstar_bus_manager_t* manager, const char* name, int cmd, void* arg);
...
{
  if (ops == NULL) {
    log_error(TAG, "Init Error", "SDIO operations pointer is NULL");
    return;
  }
  ...
  log_info(TAG, "Default Ops", "Initialized default SDIO operations");
}
                               ...
                               size_t*                    bytes_written)
{
  ...
  if (bus_config->config.sdio.ops.write) {
    ...
    log_error(TAG, "Write Error", "No write operation defined for SDIO bus '%s'", name);
                              ...
                              size_t*                    bytes_read)
{
  ...
  if (bus_config->config.sdio.ops.read) {
    ...
    log_error(TAG, "Read Error", "No read operation defined for SDIO bus '%s'", name);
    return ESP_ERR_NOT_SUPPORTED;
  }
}
...
pstar_bus_sdio_ioctl(const pstar_bus_manager_t* manager, const char* name, int cmd, void* arg)
{
  /* Validate input */
  if (!manager || !name) {
    log_error(TAG, "IOCTL Error", "Invalid parameters: manager or name is NULL");
    return ESP_ERR_INVALID_ARG;
  }
  ...
  pstar_bus_config_t* bus_config = pstar_bus_manager_find_bus(manager, name);
  if (!bus_config) {
    log_error(TAG, "IOCTL Error", "Bus '%s' not found", name);
    return ESP_ERR_NOT_FOUND;
  }
  ...
  if (bus_config->config.sdio.ops.ioctl) {
    ...
    log_error(TAG, "IOCTL Error", "No ioctl operation defined for SDIO bus '%s'", name);
    return ESP_ERR_NOT_SUPPORTED;
  }
}
                                           ...
                                           size_t*                    bytes_written)
{
  ...
  if (!card) {
    log_error(TAG, "Write Error", "No SDIO card found for bus '%s'", name);
    return ESP_ERR_INVALID_STATE;
  }

  /* Validate sector size */
  if (card->csd.sector_size == 0) {
    log_error(TAG, "Write Error", "Invalid sector size (0) for SDIO card on bus '%s'", name);
    return ESP_ERR_INVALID_STATE;
  }
  ...
  if (bus_config->config.sdio.callbacks.on_transfer_complete) {
    ...
    pstar_bus_event_t event = {.bus_type  = k_pstar_bus_type_sdio,
                               .bus_name  = name,
                               .data.sdio = {.is_write     = true,
                                             .len          = len,
                                             .offset       = offset,
                                          ...
                                          size_t*                    bytes_read)
{
  ...
  sdmmc_card_t* card = bus_config->config.sdio.card;
  if (!card) {
    log_error(TAG, "Read Error", "No SDIO card found for bus '%s'", name);
    return ESP_ERR_INVALID_STATE;
  }

  /* Validate sector size */
  if (card->csd.sector_size == 0) {
    log_error(TAG, "Read Error", "Invalid sector size (0) for SDIO card on bus '%s'", name);
    return ESP_ERR_INVALID_STATE;
  }
...
priv_pstar_bus_sdio_ioctl(const pstar_bus_manager_t* manager, const char* name, int cmd, void* arg)
{
  /* Validate input */
  if (!manager || !name) {
    log_error(TAG, "IOCTL Error", "Invalid parameters");
    return ESP_ERR_INVALID_ARG;
  }
  ...
  /* Check if the bus is initialized */
  if (!bus_config->initialized) {
    log_error(TAG, "IOCTL Error", "Bus '%s' is not initialized", name);
    return ESP_ERR_INVALID_STATE;
  }
  ...
  if (card == NULL &&
      (cmd == k_pstar_sdio_ioctl_get_csd || cmd == k_pstar_sdio_ioctl_get_cid ||
       cmd == k_pstar_sdio_ioctl_get_scr || cmd == k_pstar_sdio_ioctl_get_ocr ||
       cmd == k_pstar_sdio_ioctl_get_rca || cmd == k_pstar_sdio_ioctl_get_bus_width ||
       cmd == k_pstar_sdio_ioctl_get_bus_freq)) {
    log_error(TAG, "IOCTL Error", "Card not available for requested operation");
    return ESP_ERR_INVALID_STATE;
  }
  ...
  switch ((pstar_sdio_ioctl_cmd_t)cmd) { // Cast cmd to the enum type for clarity
    ...
    case k_pstar_sdio_ioctl_get_status:
      /* Get card status */
      if (!arg) {
        log_error(TAG, "IOCTL Error", "Argument is NULL for get_status");
        return ESP_ERR_INVALID_ARG;
      }
    ...
    case k_pstar_sdio_ioctl_get_csd:
      /* Get CSD register */
      if (!arg) {
        log_error(TAG, "IOCTL Error", "Argument is NULL for get_csd");
        return ESP_ERR_INVALID_ARG;
      }
    ...
    case k_pstar_sdio_ioctl_get_cid:
      /* Get CID register */
      if (!arg) {
        log_error(TAG, "IOCTL Error", "Argument is NULL for get_cid");
        return ESP_ERR_INVALID_ARG;
      }
      /* Card pointer is already validated above */
    ...
    case k_pstar_sdio_ioctl_get_scr:
      /* Get SCR register */
      if (!arg) {
        log_error(TAG, "IOCTL Error", "Argument is NULL for get_scr");
        return ESP_ERR_INVALID_ARG;
      }
      /* Card pointer is already validated above */
    ...
    case k_pstar_sdio_ioctl_get_ocr:
      /* Get OCR register */
      if (!arg) {
        log_error(TAG, "IOCTL Error", "Argument is NULL for get_ocr");
        return ESP_ERR_INVALID_ARG;
      }
      /* Card pointer is already validated above */
    ...
    case k_pstar_sdio_ioctl_get_rca:
      /* Get RCA register */
      if (!arg) {
        log_error(TAG, "IOCTL Error", "Argument is NULL for get_rca");
        return ESP_ERR_INVALID_ARG;
      }
      /* Card pointer is already validated above */
    ...
    case k_pstar_sdio_ioctl_get_bus_width:
      /* Get bus width */
      if (!arg) {
        log_error(TAG, "IOCTL Error", "Argument is NULL for get_bus_width");
        return ESP_ERR_INVALID_ARG;
      }
      /* Card pointer is already validated above */
    ...
    case k_pstar_sdio_ioctl_get_bus_freq:
      /* Get bus frequency */
      if (!arg) {
        log_error(TAG, "IOCTL Error", "Argument is NULL for get_bus_freq");
        return ESP_ERR_INVALID_ARG;
      }
      /* Card pointer is already validated above */
    ...
    case k_pstar_sdio_ioctl_set_bus_freq:
      ...
      log_error(TAG, "IOCTL Error", "Setting bus frequency not supported");
      return ESP_ERR_NOT_SUPPORTED;

    case k_pstar_sdio_ioctl_enable_card_detect:
      /* Enable/disable card detect is not supported in the current implementation */
      log_error(TAG, "IOCTL Error", "Card detection control not supported");
      return ESP_ERR_NOT_SUPPORTED;

    case k_pstar_sdio_ioctl_custom:
      /* Custom IOCTL - not supported */
      log_error(TAG, "IOCTL Error", "Custom IOCTL not supported");
      return ESP_ERR_NOT_SUPPORTED;

    default:
      log_error(TAG, "IOCTL Error", "Unknown IOCTL command: %d", cmd);
      return ESP_ERR_INVALID_ARG;
  }
