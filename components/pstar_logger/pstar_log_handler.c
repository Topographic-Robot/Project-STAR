/* components/pstar_logger/pstar_log_handler.c */
                  ...
                  va_list           args)
{
  ...
  if (!tag || !short_msg || !detailed_msg) {
    ESP_LOGE(TAG,
             "Invalid arguments passed to log_write_va (tag=%p, short=%p, detailed=%p)",
             tag,
             short_msg,
             detailed_msg);
    return;
  ...
  if (result < 0) {
    ...
    ESP_LOGW(tag,
             "Detailed message truncated from %d to %zu bytes",
             result,
             sizeof(formatted_msg) - 1);
    formatted_msg[sizeof(formatted_msg) - 1] = '\0'; /* Ensure null-termination */
  }

  /* Get task information */
  char task_info[CONFIG_PSTAR_KCONFIG_LOGGING_TASK_NAME_LENGTH + 10] = {
    0}; /* Slightly larger buffer */
#ifdef CONFIG_PSTAR_KCONFIG_LOGGING_INCLUDE_TASK_INFO
  ...
  char prefix_buf[sizeof(timestamp_buf) + sizeof(task_info) + 25] = {
    0}; /* Buffer for "[timestamp][seq][task]" */
  char*  p         = prefix_buf;
  ...
  if (written < 0) {
    ...
    ESP_LOGW(tag,
             "Complete message truncated from %d to %zu bytes",
             written,
             sizeof(complete_msg) - 1);
    complete_msg[sizeof(complete_msg) - 1] = '\0'; /* Ensure null-termination */
  }

  /* --- Always Log to Console (using underlying ESP log) --- */
#ifdef CONFIG_PSTAR_KCONFIG_LOGGING_CONSOLE_ENABLED
...
#endif
{
  ...
  if (file_manager == NULL && sd_card == NULL) {
    ...
    ESP_LOGE(
      TAG,
      "Full log init failed: Both file_manager and sd_card must be provided when SD logging is enabled.");
...
esp_err_t log_flush(void)
{
  ...
  if (!atomic_load(&s_logger_fully_initialized)) {
    ...
    log_warn(TAG, "Flush Error", "Log flush called but logger is not fully initialized.");
    /* Return error if SD is enabled but not initialized */
...
esp_err_t log_cleanup(void)
{
  ...
  if (storage_cleanup_ret != ESP_OK && storage_cleanup_ret != ESP_ERR_TIMEOUT) {
    ESP_LOGW(TAG,
             "Cleanup Warning - Log storage cleanup failed: %s",
             esp_err_to_name(storage_cleanup_ret));
    if (final_ret == ESP_OK) {
  ...
  if (xTaskGetSchedulerState() != taskSCHEDULER_NOT_STARTED) {
    ...
    if (current_task) {
      ...
      if (written < 0 || (size_t)written >= size) {
        if (size > 0)
          buffer[size - 1] = '\0'; /* Ensure null termination if truncated */
      }
    } else {
