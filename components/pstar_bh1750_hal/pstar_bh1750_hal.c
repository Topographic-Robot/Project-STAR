...
static esp_err_t bh1750_write_cmd(bh1750_hal_handle_t handle, uint8_t cmd)
{
...
static esp_err_t bh1750_read_data(bh1750_hal_handle_t handle, uint8_t* data, size_t len)
{
...
static uint16_t bh1750_get_delay_ms(bh1750_hal_mode_t mode)
{
  switch (mode) {
  ...
  bh1750_hal_handle_t*       out_handle)
{
  ...
  *out_handle   = NULL;
  ...
  /* 2. Set Initial Mode */
  ret = bh1750_hal_set_mode(dev, initial_mode); /* This also updates internal state */
...
esp_err_t bh1750_hal_deinit(bh1750_hal_handle_t handle, bool power_down)
{
  ...
  if (power_down) {
    esp_err_t pd_ret = bh1750_write_cmd(handle, BH1750_CMD_POWER_DOWN);
    if (pd_ret != ESP_OK) {
...
esp_err_t bh1750_hal_set_mode(bh1750_hal_handle_t handle, bh1750_hal_mode_t mode)
{
  ...
  ret = bh1750_write_cmd(handle, (uint8_t)mode);
  if (ret == ESP_OK) {
    handle->current_mode         = mode;
    handle->measurement_delay_ms = bh1750_get_delay_ms(mode);
...
esp_err_t bh1750_hal_read_lux(bh1750_hal_handle_t handle, float* lux)
{
  ...
  *lux = 0.0f; /* Default value on error */
  ...
  ret = bh1750_read_data(handle, read_buffer, 2);
  if (ret != ESP_OK) {
...
esp_err_t bh1750_hal_power_down(bh1750_hal_handle_t handle)
{
  ...
  return bh1750_write_cmd(handle, BH1750_CMD_POWER_DOWN);
}
...
esp_err_t bh1750_hal_power_on(bh1750_hal_handle_t handle)
{
  ...
  esp_err_t ret = bh1750_write_cmd(handle, BH1750_CMD_POWER_ON);
  if (ret == ESP_OK) {
    vTaskDelay(pdMS_TO_TICKS(5)); /* Small delay after power on */
  }
  return ret;
...
esp_err_t bh1750_hal_create_default(pstar_bus_manager_t* manager, bh1750_hal_handle_t* out_handle)
{
  ...
  *out_handle                           = NULL;
  ...
  ESP_LOGI(TAG, "Creating BH1750 with default configuration");
...
#error "No default BH1750 mode selected in KConfig"
  ...
  ret = bh1750_hal_init(manager, &hal_config, default_mode, out_handle);
  ESP_GOTO_ON_ERROR(ret, cleanup, TAG, "Failed to initialize BH1750 HAL: %s", esp_err_to_name(ret));

  ESP_LOGI(TAG, "BH1750 created and initialized successfully");
  return ESP_OK;
...
esp_err_t register_bh1750_pins(void)
{
  ...
  esp_err_t ret = ESP_OK;
  ESP_LOGI(TAG, "Registering BH1750 pins with validator...");

  /* Register SDA pin */
  const char* sda_desc = "BH1750 SDA";
  ret                  = pstar_register_pin(CONFIG_PSTAR_KCONFIG_BH1750_SDA_PIN,
                           sda_desc,
                           true /* I2C pins can be shared */);
  if (ret != ESP_OK) {
    ESP_LOGE(TAG,
             "Failed to register BH1750 SDA pin (%d): %s",
             CONFIG_PSTAR_KCONFIG_BH1750_SDA_PIN,
             esp_err_to_name(ret));
    return ret;
  }
  ...
  ret                  = pstar_register_pin(CONFIG_PSTAR_KCONFIG_BH1750_SCL_PIN,
                           scl_desc,
                           true /* I2C pins can be shared */);
  if (ret != ESP_OK) {
    ESP_LOGE(TAG,
             "Failed to register BH1750 SCL pin (%d): %s",
             CONFIG_PSTAR_KCONFIG_BH1750_SCL_PIN,
             esp_err_to_name(ret));
    /* Note: If SDA succeeded but SCL failed, SDA remains registered. */
    /* Consider adding cleanup/deregistration logic if needed, but usually */
    /* pstar_validate_pins will catch the overall state. */
    return ret;
  }

  ESP_LOGI(TAG, "BH1750 pins registered successfully.");
  return ESP_OK;
#else
  ESP_LOGD(TAG, "Pin validator disabled, skipping BH1750 pin registration.");
