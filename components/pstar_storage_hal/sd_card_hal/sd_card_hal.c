...
static const sd_card_pin_config_t sd_card_default_pins = {
  ...
  .sdio_d3_pin   = GPIO_NUM_13, /* SDIO D3 (only for 4-bit mode) */
};
...
esp_err_t sd_card_cleanup(sd_card_hal_t* sd_card)
{
  ...
  if (sd_card->mount_task_handle != NULL) {
    ...
    /* Give some time for task to exit gracefully */
    vTaskDelay(pdMS_TO_TICKS(100));
  }

  /* Take mutex for thread-safe access */
  if (sd_card->mutex != NULL) {
    if (xSemaphoreTake(sd_card->mutex, portMAX_DELAY) == pdTRUE) {
      mutex_taken = true;
    } else {
      log_error(sd_card->tag, 
                "Cleanup Error", 
                "Failed to acquire mutex for cleanup");
      /* Continue with cleanup even if we can't get the mutex */
    }
  }

  /* Unmount if mounted */
  if (mutex_taken && sd_card->card_available) {
  ...
  /* Clean up GPIO ISR */
  if (mutex_taken && sd_card->initialized) {
    pstar_bus_gpio_isr_remove(&sd_card->bus_manager, 
                        sd_card_default_gpio_bus_name, 
                        sd_card->pin_config.gpio_det_pin);
  }
  ...
  /* Free card structure */
  if (mutex_taken && sd_card->card != NULL) {
    free(sd_card->card);
    sd_card->card = NULL;
  }
...
/**
 * @brief Measures SD card performance metrics
 * 
 * @param[in] sd_card Pointer to the SD card HAL instance
 */
static void priv_measure_card_performance(sd_card_hal_t* sd_card)
{
  ...
  snprintf(test_path, sizeof(test_path), "%s/.sd_perf_test", sd_card->mount_path);
  
  /* Check if path is safe before proceeding */
  if (!priv_path_is_safe_simple(test_path) || !priv_is_path_within_mount(sd_card, test_path)) {
    log_error(sd_card->tag,
              "Performance Error",
              "Test file path '%s' is not valid",
              test_path);
    free(buffer);
    return;
  }
      ...
      if (debounce_counter == 0) {
        ...
        if (xSemaphoreTake(sd_card->mutex, sd_card->task_config.mutex_timeout) == pdTRUE) {
          ...
          if (is_inserted && !sd_card->card_available) {
            ...
            /* Card removed - update state machine first */
            priv_update_state_machine(sd_card, k_sd_state_idle);
